...
def simulate_zombie_outbreak(city, days):
    ...
    day = 1
    while day <= days:
        today = copy.deepcopy(city.grid)
        for row in range(0, city.rows):
            for column in range(0, city.cols):
                if city.grid[row][column] == 'Z':
                    # left to Z if H and in range
                    if column > 0:
                        if city.grid[row][column-1] == 'H':
                            today[row][column-1] = 'Z'
                    # right to Z if H and in range
                    if column < city.cols - 1:
                        if city.grid[row][column+1] == 'H':
                            today[row][column+1] = 'Z'
                    # above to Z if and and in range
                    if row > 0:
                        if city.grid[row-1][column] == 'H':
                            today[row-1][column] = 'Z'
                    # below to Z if H and in range
                    if row < city.rows - 1:
                        if city.grid[row+1][column] == 'H':
                            today[row+1][column] = 'Z'
        day += 1
